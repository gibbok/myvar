...
def get_llm(temp=0.7):
    ...
    return ChatGoogleGenerativeAI(
        model=GEMINI_MODEL, api_key=GEMINI_API_KEY, temperature=temp
    )
...
def slugify(text: str) -> str:
    ...
    return re.sub(
        r"-+", "-", re.sub(r"[^a-z0-9\s-]", "", text.lower()).replace(" ", "-")
    ).strip("-")
...
def reviewer_node(state: AgentState):
    ...
    decision = (
        ...
        if decision_match
        else ("APPROVE" if "APPROVE" in res.upper() else "REVISE")
    )
    ...
    if state["count"] >= 4:
        decision = "APPROVE"  # Force exit to prevent hanging/loops
...
def publisher_node(state: AgentState):
    ...
    # Simple extraction
    title_raw = (
        re.search(r"TITLE:\s*(.*)", res, re.I).group(1).strip()
        if "TITLE:" in res
        else state["title"]
    )
    ...
    topic = (
        re.search(r"TOPIC:\s*(.*)", res, re.I).group(1).strip()
        if "TOPIC:" in res
        else "content"
    )
    tags_raw = (
        re.search(r"TAGS:\s*(.*)", res, re.I).group(1).strip()
        if "TAGS:" in res
        else "tech"
    )
    ...
    desc = (
        re.search(r"DESC:\s*(.*)", res, re.I).group(1).strip()
        if "DESC:" in res
        else "No description."
    )
    ...
    with open(path, "w") as f:
        f.write(frontmatter + state["content"])
